...
export class SortbankComponent implements OnInit {
  ...
  exportTable() {
    const worksheet: XLSX.WorkSheet = XLSX.utils.table_to_sheet(document.getElementById('exportableTable')); // 将这个表格转换成一个 sheet
    const workbook: XLSX.WorkBook = { Sheets: { 'data': worksheet }, SheetNames: ['data'] };
    const excelBuffer: any = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    // 这里类型如果不正确，下载出来的可能是类似xml文件的东西或者是类似二进制的东西等
    this.saveAsExcelFile(excelBuffer, 'SortBank监控');
  ...
  saveAsExcelFile(buffer: any, fileName: string) {
    ...
    FileSaver.saveAs(data, fileName + '_export_' + this.option.params.timekey + '.xls');
