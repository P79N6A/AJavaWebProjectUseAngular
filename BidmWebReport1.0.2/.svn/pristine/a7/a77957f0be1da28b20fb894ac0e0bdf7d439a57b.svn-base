...
public class Monitor8ServiceImple implements Monitor8Service {
			...
			if(wb != null) {  //判断文件是否是excel格式
				...
				if(sheet != null) { // 判断第一个sheet是否有内容
					...
					for (int i = sheet.getFirstRowNum()+1; i <= sheet.getLastRowNum();i++){
						...
						if(datenamestr == null || datenamestr.equals("")){ // 当为 空的时候， 直接赋值为空
							monitor8vo.setDATENAME(null);
						}else{
						...
						if (hourtimekeystr == null || hourtimekeystr.equals("")) {
							monitor8vo.setHOURTIMEKEY(null);
						}else{
						...
						monitor8vo.setOwner(row.getCell(firstCell+8) != null ? String.valueOf(row.getCell(firstCell+8)).trim():null);
						if(monitor8vo != null) {
							System.out.println(monitor8vo);
	...
	//下面这是往数据库里面写入数据的操作：先把，已经存在的 factory和 datename 的数据删除掉，然后再把新的数据重新写入进去 
	@Override
	public int insert_ExcelImport(List<Monitor8VO> Monitor8VOList){
		...
		try {
			for(Object obj:Monitor8VOList) {
				...
				Monitor8VO monitor8vo_ = new Monitor8VO(); // 这个是要进行插入的对象
				...
				monitor8vo_.setOwner(monitor8.get("owner") !=null?monitor8.get("owner").toString():"");
				//保存起来 留着插入使用
				...
				deletemonitor8vo.setOwner(monitor8vo_.getOwner());
		...
		while (iterator.hasNext()) {
			Monitor8VO monitor8vo = iterator.next();
			try {
		...
		while (iterator2.hasNext()) {
			Monitor8VO monitor8vo = iterator2.next();
			try {
